...
-define(LEFT_X, 250). %% Defines the left-most X of this field
-define(RIGHT_X, 500). %% Defines the right-most X of this field (could be calculated by LEFT_X + FIELD_WIDTH)
...
initGame(GameID) ->
	...
	ets:insert(gameStat, {p8_A_1, 'A', GameID, 8, ?FIELD_WIDTH/4 + 50, ?FIELD_HEIGHT/2 - 100, true, false}), 				% inserts the GK
	ets:insert(gameStat, {p9_A_1, 'A', GameID, 9, ?FIELD_WIDTH/4 + 50, ?FIELD_HEIGHT/2 + 100, true, false}), 				% inserts the 
	ets:insert(gameStat, {p10_A_1, 'A', GameID, 10, ?FIELD_WIDTH/2 - 100, ?FIELD_HEIGHT/2 - 50, true, false}), 				% inserts the 
	ets:insert(gameStat, {p11_A_1, 'A', GameID, 11, ?FIELD_WIDTH/2 - 50, ?FIELD_HEIGHT/2 + 50, true, false}), 				% inserts the 
	...
	end, [{p8_A_1, 'A', 8}, {p9_A_1, 'A', 9}, {p10_A_1, 'A', 10}, {p11_A_1, 'A', 11}]).
...
getNeighbourNodesRef() -> 
	[LeftNeighbour|_] = ets:lookup_element(?IP_MAP, n1, 2),
	[RightNeighbour|_] = ets:lookup_element(?IP_MAP, n3, 2),
